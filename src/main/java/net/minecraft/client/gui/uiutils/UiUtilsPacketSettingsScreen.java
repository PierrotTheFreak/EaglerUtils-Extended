...
/** Searchable, directional packet-delay configuration. Packets remain held until Delay Packets is disabled. */
public class UiUtilsPacketSettingsScreen extends Screen {
    private static final int W=360, TOP=55, ROW=18, VISIBLE=10;
    ...
    @Override protected void init(){int left=getLeft();searchField=new TextFieldWidget(font,left+10,34,W-20,20,"Search packets...");searchField.setMaxStringLength(128);addButton(searchField);searchField.setFocused(true);setFocused(searchField);scroll=0;}
    ...
    @Override public boolean keyPressed(int keyCode,int scanCode,int modifiers){if(keyCode==256){closeScreen();return true;}if(searchField!=null&&searchField.isFocused()&&searchField.keyPressed(keyCode,scanCode,modifiers))return true;return super.keyPressed(keyCode,scanCode,modifiers);}
