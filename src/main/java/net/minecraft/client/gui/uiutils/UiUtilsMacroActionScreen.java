...
public class UiUtilsMacroActionScreen extends Screen {
    ...
    private UiUtilsMacroAction editingAction;
    ...
    private UiUtilsMacroAction currentAction(){UiUtilsMacroAction a=editingAction==null?new UiUtilsMacroAction(selectedType):editingAction;a.setType(selectedType);a.setText(valueField==null?"":valueField.getText());try{a.setAmount(Integer.parseInt(amountField==null?"0":amountField.getText().trim()));}catch(Exception ignored){a.setAmount(0);}a.setEnabled(enabled);return a;}
    @Override public boolean mouseClicked(double mx,double my,int button){
        ...
        if(button==0){int left=getPanelLeft();
            ...
            if(selectedType==UiUtilsMacroAction.Type.DELAY_PACKETS&&inside(mx,my,left+220,135,180,24)){editingAction=currentAction();mc.displayGuiScreen(new UiUtilsMacroPacketScreen(this,editingAction,actions));return true;}
            if(inside(mx,my,left+10,250,130,24)){UiUtilsMacroAction a=currentAction();if(editingAction==null)actions.add(a);else if(!actions.contains(editingAction))actions.add(editingAction);mc.displayGuiScreen(parent);return true;}
    ...
    @Override public void render(int mx,int my,float partialTicks){renderBackground();int left=getPanelLeft();fill(left,0,left+PANEL_WIDTH,PANEL_HEIGHT,0xEE111111);drawCenteredString(font,"Add Macro Action",width/2,10,0xFFFFFF);drawString(font,"Action Type:",left+10,63,0xAAAAAA);drawButton(left+10,105,200,24,selectedType.name(),mx,my);drawString(font,"Text:",left+10,61,0xAAAAAA);drawString(font,"Amount:",left+10,86,0xAAAAAA);
        ...
        if(selectedType==UiUtilsMacroAction.Type.DELAY_PACKETS){drawButton(left+220,135,180,24,"Configure Packets",mx,my);drawString(font,"Packets stay delayed until this action is turned off.",left+10,165,0xAAAAAA);}
