...
public class UiUtils {
    ...
    private UiUtils(){}
    public static boolean shouldShow(Screen screen){return screen instanceof ContainerScreen;}
    public static void init(Screen screen){chatInput=null;if(shouldShow(screen)){int inputY=PANEL_Y+getPanelHeight()-STATUS_HEIGHT-STATUS_GAP-CHAT_INPUT_HEIGHT;chatInput=new TextFieldWidget(screen.mc.fontRenderer,CONTROL_X,inputY,CONTROL_WIDTH,CHAT_INPUT_HEIGHT,"Chat");chatInput.setMaxStringLength(256);chatInput.setText("");chatInput.setFocused(false);}}
    public static void tick(){if(chatInput!=null)chatInput.tick();}
    public static TextFieldWidget getChatInput(){return chatInput;}
    ...
    private static boolean isInside(double mx,double my,int x,int y,int w,int h){return mx>=x&&mx<x+w&&my>=y&&my<y+h;}
    ...
    private static void toggleDelayPackets(Screen screen){if(screen.mc==null||!(screen.mc.player instanceof ClientPlayerEntity))return;ClientPlayerEntity p=(ClientPlayerEntity)screen.mc.player;if(p.connection==null)return;UiUtilsPacketManager.toggleDelayPackets(p.connection.getNetworkManager());}
    private static void openDelaySettings(Screen screen){if(screen.mc!=null)screen.mc.displayGuiScreen(new UiUtilsPacketSettingsScreen(screen));}
