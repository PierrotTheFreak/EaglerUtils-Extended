...
public class UiUtilsPacketInspectorScreen extends Screen {
    ...
    private List<UiUtilsPacketInspector.Entry> filteredEntries() {
        ...
        for (UiUtilsPacketInspector.Entry entry : UiUtilsPacketInspector.getEntries()) {
            if (search.isEmpty() || entry.name.toLowerCase().contains(search)) result.add(entry);
        }
        return result;
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        int x = left();
        if (button == 0) {
            if (inside(mouseX, mouseY, x + 270, 38, 100, 20)) {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        List<UiUtilsPacketInspector.Entry> entries = filteredEntries();
        scroll -= (int) delta;
        int max = Math.max(0, entries.size() - 10);
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (searchField != null && searchField.isFocused() && searchField.keyPressed(keyCode, scanCode, modifiers)) return true;
        return super.keyPressed(keyCode, scanCode, modifiers);
    }

    @Override
    public boolean charTyped(char codePoint, int modifiers) {
        if (searchField != null && searchField.isFocused() && searchField.charTyped(codePoint, modifiers)) return true;
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        button(x + 270, 38, 100, "Clear", mouseX, mouseY);
        ...
        for (int i = start; i < end; ++i) {
            ...
            int y = 64 + (i - start) * 18;
            fill(x + 10, y, x + 370, y + 17, 0xFF333333);
            String dir = e.direction == PacketDirection.CLIENTBOUND ? "S -> C" : "C -> S";
            drawString(font, dir, x + 14, y + 5, e.direction == PacketDirection.CLIENTBOUND ? 0x55AAFF : 0x55FF55);
            drawString(font, e.name, x + 62, y + 5, 0xFFFFFF);
            drawString(font, timeFormat.format(new Date(e.timestamp)), x + 220, y + 5, 0x888888);
            drawString(font, e.size + " B", x + 332, y + 5, 0xAAAAAA);
        }
        ...
        drawString(font, "Showing: " + entries.size() + "/" + UiUtilsPacketInspector.MAX_ENTRIES, x + 275, 252, 0xAAAAAA);
