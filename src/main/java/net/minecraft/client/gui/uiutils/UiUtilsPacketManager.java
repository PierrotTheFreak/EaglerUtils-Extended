...
public final class UiUtilsPacketManager {
    ...
    private static Class<?> findPacketClass(String name, PacketDirection direction) {
        ...
        for (Class<? extends IPacket<?>> c : getAllPacketClasses(direction)) if (c.getSimpleName().equals(name)) return c;
        return null;
    }
    ...
    public static Set<String> getSelectedPacketTypes(PacketDirection direction) {
        ...
        for (String name : getAllPacketTypes(direction)) if (isPacketDelayed(name, direction)) result.add(name);
        return result;
    }
    ...
    public static void delayAllPackets(PacketDirection direction) {
        Set<Class<?>> set = selection(direction);
        set.clear();
        for (Class<? extends IPacket<?>> c : getAllPacketClasses(direction)) if (!isKeepAlivePacket(c)) set.add(c);
    }
    ...
    public static boolean handleIncomingPacket(IPacket<?> packet) {
        UiUtilsPacketInspector.record(packet, PacketDirection.CLIENTBOUND);
    ...
    public static boolean handleOutgoingPacket(IPacket<?> packet, NetworkManager networkManager) {
        ...
        UiUtilsPacketInspector.record(packet, PacketDirection.SERVERBOUND);
    ...
    public static void disconnectAndSend(ClientPlayerEntity player) {
        ...
        flush(networkManager); flushIncomingPackets(); clearQueue();
