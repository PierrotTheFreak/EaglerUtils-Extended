...
public final class UiUtilsPacketManager {
    ...
    public static long getDelayMilliseconds(){return delayMilliseconds;}
    public static void setDelayMilliseconds(long milliseconds){delayMilliseconds=Math.max(0L,Math.min(60000L,milliseconds));}
    ...
    public static boolean handleIncomingPacket(IPacket<?> packet,Runnable processLater){if(!handleIncomingPacket(packet))return false;delayedIncomingTasks.add(new DelayedIncoming(processLater,System.currentTimeMillis()+delayMilliseconds));return true;}
